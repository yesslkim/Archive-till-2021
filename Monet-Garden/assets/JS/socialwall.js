...
const originaluserIDs = userIDs.textContent;
...
const socialWallUI = (dataLists) => {
    ...
    <tr>
      ...
      <td class="user-id">${post.userID.split('@')[0]}</td>
      <td class="written-date">${post.date.substr(2)}</td>
      <td>${post.views}</td>
    </tr>
    `
	})
              ...
              `
				...
				db.collection('posts').orderBy('createdAt').get().then(snapshot => {
					...
					posts.forEach((post, index) => {
						const postIndex = (index + 1).toString();
						const targetPost = post.data()
						if (targetPostIndex === postIndex && targetPostUserID === auth.currentUser.email.split('@')[0]) {
							postModal.innerHTML = `
              <button type="button" class="close_btn">닫기</button>
